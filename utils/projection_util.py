...
class Realistic_Projection:
    ...
    def __init__(self):
        _views = np.asarray([
            [[0 * np.pi / 4, 0, 1*np.pi / 2], [-0.5, -0.5, TRANS]]
        ])

        # adding some bias to the view angle to reveal more surface
        _views_bias = np.asarray([
            # [[0, 0 * np.pi / 3, 0], [-0.5, 0, TRANS]]
            [[0, -1.2*np.pi / 3, 0], [-0.5, 0, TRANS]]
        ])

        self.num_views = _views.shape[0]

        angle = torch.tensor(_views[:, 0, :]).float().cuda()
        # angle = torch.tensor([[1.56351295, 0.36142174, 1.93213787]]).float().cuda()
    ...
    def get_img(self, points):
        ...
        grid = points2grid(points=_points, resolution=params['resolution'], depth=params['depth'])
